...
export MESA_LOADER_DRIVER_OVERRIDE=zink TURNIP_KMD=kgsl GALLIUM_DRIVER=zink
...
setsid nohup dbus-run-session -- kwin_wayland --xwayland --socket wayland-0 \
    >/root/kwin-zink.log 2>&1 </dev/null &
...
setsid nohup "$TERM_BIN" >/root/term-zink.log 2>&1 </dev/null &
sleep 4
