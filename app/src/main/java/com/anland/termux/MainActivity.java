...
public class MainActivity extends Activity implements SurfaceHolder.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupFullscreen();
        setupCursorHiding();
        applyIntentConnectionOverrides(getIntent());
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        setIntent(intent);
        applyIntentConnectionOverrides(intent);
        if (surfaceReady) {
            nativeStop();
    ...
    private void applyIntentConnectionOverrides(Intent intent) {
        if (intent == null) return;
        SharedPreferences.Editor editor =
            getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit();
        boolean changed = false;

        String socketPath = intent.getStringExtra(KEY_SOCKET_PATH);
        if (socketPath != null && !socketPath.trim().isEmpty()) {
            editor.putString(KEY_SOCKET_PATH, socketPath.trim());
            changed = true;
        }
        if (intent.hasExtra(KEY_USE_ROOT)) {
            editor.putBoolean(KEY_USE_ROOT, intent.getBooleanExtra(KEY_USE_ROOT, false));
            changed = true;
        }
        if (changed)
            editor.apply();
